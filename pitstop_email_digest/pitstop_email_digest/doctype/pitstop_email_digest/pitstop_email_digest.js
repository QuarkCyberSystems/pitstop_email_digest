...
// frappe.ui.form.on("Pitstop Email Digest", {
// 	refresh(frm) {
...
cur_frm.cscript.refresh = function (doc, dt, dn) {
	...
	cur_frm.add_custom_button(
		...
		function () {
			frappe.call({
				method: "pitstop_email_digest.pitstop_email_digest.doctype.pitstop_email_digest.pitstop_email_digest.get_digest_msg",
				args: {
					name: doc.name,
				},
				callback: function (r) {
...
cur_frm.cscript.addremove_recipients = function (doc, dt, dn) {
	...
	return cur_frm.call("get_users", null, function (r) {
		...
		$.each(r.user_list, function (i, v) {
			...
			if (v.enabled == 0) {
				fullname = repl("<span style='color: red'> %(name)s (" + __("disabled user") + ")</span>", {
					name: v.name,
				});
			}
