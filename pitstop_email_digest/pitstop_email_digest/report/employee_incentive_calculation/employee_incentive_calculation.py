...
def execute(filters=None):
    ...
    if filters.get("based_on") == "Technician":
        ...
        # print(workshop_turnover_report_data)
...
def fetch_revenue_service_advisors(filters):
    WorkshopTurnoverReport()
    pass
            ...
            if workshop_turnover_report_data:
                for each_turnover_data in workshop_turnover_report_data:
                    for each_group_data in each_turnover_data.rows:
                        if each_group_data.get("group_value") == totals_dict.get(
                            "service_advisor"
                        ):
                            totals_dict["total_sales_amount"] = flt(
                                each_group_data.totals.get("total_sales_amount"), 3
                            )
                            # Revenue section
