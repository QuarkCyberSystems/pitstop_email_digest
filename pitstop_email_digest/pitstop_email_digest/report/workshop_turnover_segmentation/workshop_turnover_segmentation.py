...
from frappe.utils import getdate, nowdate, cstr, flt, cint, combine_datetime
...
from erpnext.accounts.report.gross_profit.gross_profit import update_item_valuation_rates
import datetime
...
class WorkshopTurnoverReport:
	...
	def get_conditions(self):
		...
		if self.filters.get("segmentation"):
			if self.filters.get("segmentation") == 'BRAC-QIC':
				conditions.append("p.customer_group = 'Budget Mobility' and " \
	...
	def set_throughput_and_open_repair_orders(self, totals, grouped_by):
		...
		vehicle_throughput = frappe.db.sql(f"""
			select count(*)
			from `tabProject` p
		...
		open_repair_orders = frappe.db.sql(f"""
			select count(*)
			from `tabProject` p
		...
		if len(self.group_by) <= 1:
			...
			if self.filters.totals_only:
				columns = [c for c in columns if c.get('fieldname') not in (
					'final_invoice_date', 'project', 'sales_invoice',
					'vehicle_workshop', 'branch', 'vehicle_workshop_division', 'cost_center', 'service_advisor', 'customer_group',
				)]
